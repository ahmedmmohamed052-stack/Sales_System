...
(function () {
  ...
  if (contactForm) {
    contactForm.addEventListener('submit', (e) => {
      ...
      const store = document.getElementById('contactStore').value;
      const message = document.getElementById('contactMessage').value.trim();

      // Basic validation
      if (!name || !email || !store || !message) {
        shakeForm(contactForm);
        return;
      }

      if (!isValidEmail(email)) {
        highlightField(document.getElementById('contactEmail'));
        return;
      }

      // Build mailto URL
      const subject = encodeURIComponent(`ForecastIQ Inquiry from ${name} — ${store} Store`);
      const body = encodeURIComponent(
        `Hi Ahmed,\n\nMy name is ${name} and I'm interested in the AI Sales Forecasting system for my ${store} store.\n\n---\n\n${message}\n\n---\n\nReply to: ${email}`
      );

      const mailtoUrl = `mailto:ahmedmmohamed052@gmail.com?subject=${subject}&body=${body}`;
  ...
  function shakeForm(form) {
    ...
    form.style.animation = 'shake 0.45s var(--ease-out) both';
